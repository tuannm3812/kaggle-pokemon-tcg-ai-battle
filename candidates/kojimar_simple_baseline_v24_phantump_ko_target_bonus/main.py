...
class LucarioPolicy:
    ...
    def _phantump_ko_bonus(self, pokemon: Pokemon) -> int:
        if not self._opponent_is_phantump_control():
            return 0
        if pokemon.id == C.HOPS_PHANTUMP:
            return 1800
        if pokemon.id == C.HOPS_TREVENANT:
            return 1200
        if pokemon.id == C.HOPS_CRAMORANT:
            return 550
        if pokemon.id == C.HOPS_SNORLAX:
            return 350
        return 0
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    if prize == 0:
                        ...
                        score += self._phantump_ko_bonus(op_pokemon)
