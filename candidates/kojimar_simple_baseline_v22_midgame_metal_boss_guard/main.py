...
class LucarioPolicy:
    ...
    def _opponent_active_is_metal_threat(self) -> bool:
        if not self.opponent.active:
            return False
        return self.opponent.active[0].id in {C.DURALUDON, C.ARCHALUDON_EX, C.CINDERACE}

    def _should_guard_midgame_metal_boss(self) -> bool:
        if not self._opponent_is_metal_cinderace():
            return False
        if not (5 <= self.state.turn <= 9):
            return False
        if not self._opponent_active_is_metal_threat():
            return False
        if plan.target < 1:
            return False
        return plan.remain_hp > 0
    ...
    def _score_play_trainer(self, card: Card) -> float:
        ...
        if card.id == C.BOSS_ORDERS:
            if plan.target < 1:
                return -1
            if self._should_guard_midgame_metal_boss():
                return -1
            return 3200
