...
class LucarioPolicy:
    ...
    def _opponent_is_strict_public_boss_guard_family(self) -> bool:
        # v8 was too broad because Dunsparce appears in many non-Alakazam decks.
        # Guard speculative Boss only when a true Metal/Cinderace or Alakazam
        # evolution line is visible.
    ...
    def _score_play_trainer(self, card: Card) -> float:
        ...
        if card.id == C.BOSS_ORDERS:
            if plan.target < 1:
                return -1
            if self._opponent_is_strict_public_boss_guard_family() and plan.remain_hp > 0:
                return -1
            return 3200
