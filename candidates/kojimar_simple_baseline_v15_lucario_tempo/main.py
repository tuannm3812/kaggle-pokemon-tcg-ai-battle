...
class LucarioPolicy:
    ...
    def _opponent_needs_lucario_tempo(self) -> bool:
        return self._opponent_has({
            C.DREEPY, C.DRAKLOAK, C.DRAGAPULT_EX,
            C.HOPS_PHANTUMP, C.HOPS_TREVENANT,
        })
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if self._opponent_needs_lucario_tempo():
            if pokemon.id == C.RIOLU:
                score += 550 if energy_count < 2 else 50
            elif pokemon.id == C.MEGA_LUCARIO_EX:
                score += 650 if energy_count < 2 else 100
            elif pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
                score -= 300
