...
class LucarioPolicy:
    ...
    def _opponent_active_is_midgame_threat(self) -> bool:
        if not self.opponent.active:
            return False
        active_id = self.opponent.active[0].id
        if self._opponent_is_metal_cinderace() and active_id in {C.ARCHALUDON_EX, C.DURALUDON, C.CINDERACE}:
            return True
        if self._opponent_is_alakazam_dunsparce() and active_id in {C.ALAKAZAM, C.KADABRA, C.ABRA, C.DUNSPARCE, C.DUDUNSPARCE}:
            return True
        return False

    def _should_prioritize_midgame_ko_attack(self) -> bool:
        if not (5 <= self.state.turn <= 9):
            return False
        if not self.me.active or self.me.active[0].id != C.MEGA_LUCARIO_EX:
            return False
        if not self._opponent_active_is_midgame_threat():
            return False
        if plan.attacker != 0 or plan.target != 0 or plan.attack_index < 0:
            return False
        if plan.remain_hp > 0:
            return False
        return len(self.me.active[0].energies) >= 1
    ...
    def _score_option(self, option) -> float:
        ...
        if option.type == OptionType.ATTACK:
            if self._should_prioritize_midgame_ko_attack():
