...
class LucarioPolicy:
    ...
    def _active_threat_finish_bonus(self, pokemon: Pokemon, damage: int, target_index: int) -> int:
        """Active-only finish pressure for weak public midgames.

        Unlike v18, this never rewards bench targets, so it cannot create
        additional Boss usage. It only makes the attack planner prefer
        finishing or heavily pressuring the already-active evolved threat.
        """
        if target_index != 0 or not (5 <= self.state.turn <= 9):
            return 0
        remaining_hp = pokemon.hp - damage
        will_ko = remaining_hp <= 0
        near_finish = remaining_hp <= 160

        if self._opponent_is_metal_cinderace():
            if pokemon.id == C.ARCHALUDON_EX:
                return 1250 if will_ko else 650 if near_finish else 0
            if pokemon.id == C.DURALUDON:
                return 900 if will_ko else 350 if near_finish else 0
            if pokemon.id == C.CINDERACE:
                return 700 if will_ko else 220 if near_finish else 0

        if self._opponent_is_alakazam_dunsparce():
            if pokemon.id == C.ALAKAZAM:
                return 1150 if will_ko else 580 if near_finish else 0
            if pokemon.id == C.KADABRA:
                return 850 if will_ko else 300 if near_finish else 0
            if pokemon.id in {C.ABRA, C.DUNSPARCE, C.DUDUNSPARCE}:
                return 650 if will_ko else 180 if near_finish else 0
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    score += self._active_threat_finish_bonus(op_pokemon, damage, target_index)
                    score += base_score
