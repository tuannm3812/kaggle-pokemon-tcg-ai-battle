...
class LucarioPolicy:
    ...
    def _meta_target_bonus(self, pokemon: Pokemon, damage: int, target_index: int) -> int:
        """Small public-meta target priorities layered on top of v1's KO-first plan."""
        bonus = 0
        will_ko = pokemon.hp <= damage

        if self._opponent_is_metal_cinderace():
            if pokemon.id == C.ARCHALUDON_EX:
                bonus += 650
            elif pokemon.id == C.CINDERACE:
                bonus += 550
            elif pokemon.id == C.DURALUDON:
                bonus += 450 if will_ko else 220
            elif pokemon.id == C.RELICANTH and len(pokemon.energies) >= 1:
                bonus += 220

        if self._opponent_is_alakazam_dunsparce():
            if pokemon.id == C.ABRA:
                bonus += 900 if will_ko else 350
            elif pokemon.id == C.KADABRA:
                bonus += 750 if will_ko else 300
            elif pokemon.id == C.ALAKAZAM:
                bonus += 650
            elif pokemon.id == C.DUNSPARCE:
                bonus += 500 if will_ko else 180
            elif pokemon.id == C.DUDUNSPARCE:
                bonus += 350

        if self._opponent_is_phantump_control():
            if pokemon.id == C.HOPS_PHANTUMP:
                bonus += 700 if will_ko else 120
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    score += self._meta_target_bonus(op_pokemon, damage, target_index)
                    score += base_score
