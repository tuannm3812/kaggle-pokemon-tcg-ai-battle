...
class LucarioPolicy:
    ...
    def _opponent_needs_fast_lucario_opening(self) -> bool:
        return self._opponent_is_metal_cinderace() or self._opponent_is_alakazam_dunsparce()
    ...
    def _score_setup_active(self, card: Pokemon | Card) -> int:
        if self._opponent_needs_fast_lucario_opening():
            if card.id == C.RIOLU:
                return 8
