...
class LucarioPolicy:
    ...
    def _opponent_is_public_boss_guard_family(self) -> bool:
        return self._opponent_has({
            C.DURALUDON, C.ARCHALUDON_EX, C.CINDERACE,
            C.ABRA, C.KADABRA, C.ALAKAZAM, C.DUNSPARCE, C.DUDUNSPARCE,
        })

    def _opponent_is_dragapult(self) -> bool:
        return self._opponent_has({C.DREEPY, C.DRAKLOAK, C.DRAGAPULT_EX})

    def _target_score(self, pokemon: Pokemon) -> int:
        score = target_score(pokemon)
        if self._opponent_is_dragapult():
            if pokemon.id == C.DREEPY:
                score += 900
            elif pokemon.id == C.DRAKLOAK:
                score += 700
            elif pokemon.id == C.DRAGAPULT_EX:
                score += 300
            elif pokemon.id == C.BUDEW:
                score += 350
        return score
    ...
    def _score_play_trainer(self, card: Card) -> float:
        ...
        if card.id == C.BOSS_ORDERS:
            if plan.target < 1:
                return -1
            if self._opponent_is_public_boss_guard_family() and plan.remain_hp > 0:
                return -1
            return 3200
