...
class LucarioPolicy:
    ...
    def _opponent_is_metal_cinderace(self) -> bool:
        return self._opponent_has({C.DURALUDON, C.ARCHALUDON_EX, C.CINDERACE})
    ...
    def _target_score(self, pokemon: Pokemon) -> int:
        score = target_score(pokemon)

        # Public v5 episodes showed the Metal/Cinderace pressure transfers, but
        # Relicanth can become a distraction. Keep pressure on the main engine
        # pieces only when the archetype is visible.
        if self._opponent_is_metal_cinderace():
            if pokemon.id == C.ARCHALUDON_EX:
                ...
                score += 450
            elif pokemon.id == C.RELICANTH:
                score += 50

        # Kojimar v1 and v5 both underperform into Alakazam/Dunsparce. Prefer
        # removing the evolving Psychic line before it stabilizes; keep support
        # Dunsparce targets valuable but below Abra/Kadabra tempo targets.
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    score = self._target_score(op_pokemon)
