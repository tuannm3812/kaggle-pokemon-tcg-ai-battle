...
class LucarioPolicy:
    ...
    def _opponent_is_metal_cinderace(self) -> bool:
        return self._opponent_has({C.RELICANTH, C.DURALUDON, C.ARCHALUDON_EX, C.CINDERACE})

    def _opponent_is_alakazam_dunsparce(self) -> bool:
        return self._opponent_has({C.ABRA, C.KADABRA, C.ALAKAZAM, C.DUNSPARCE, C.DUDUNSPARCE})

    def _in_midgame_finish_window(self) -> bool:
        return 5 <= self.state.turn <= 7

    def _midgame_finish_bonus(self, pokemon: Pokemon, damage: int, target_index: int) -> int:
        """Conservative replay-derived pressure for weak public midgames.

        This intentionally does not change setup, energy routing, or generic Boss
        timing. It only nudges attack planning when a weak-family target is
        already KO-able or nearly converted in turns 5-7.
        """
        if not self._in_midgame_finish_window():
            return 0

        remaining_hp = pokemon.hp - damage
        will_ko = remaining_hp <= 0
        near_ko_active = target_index == 0 and remaining_hp <= 130
        bonus = 0

        if self._opponent_is_metal_cinderace():
            if pokemon.id == C.ARCHALUDON_EX and (will_ko or near_ko_active):
                bonus += 700 if will_ko else 320
            elif pokemon.id == C.DURALUDON and will_ko:
                bonus += 520
            elif pokemon.id == C.CINDERACE and will_ko:
                bonus += 420
            elif pokemon.id == C.RELICANTH and will_ko and len(pokemon.energies) >= 1:
                bonus += 260

        if self._opponent_is_alakazam_dunsparce():
            if pokemon.id == C.ALAKAZAM and (will_ko or near_ko_active):
                bonus += 650 if will_ko else 260
            elif pokemon.id == C.KADABRA and will_ko:
                bonus += 560
            elif pokemon.id == C.ABRA and will_ko:
                bonus += 500
            elif pokemon.id == C.DUNSPARCE and will_ko:
                bonus += 380
            elif pokemon.id == C.DUDUNSPARCE and will_ko:
                bonus += 300

        if target_index >= 1 and not will_ko:
            bonus -= 220
        return bonus
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    score += self._midgame_finish_bonus(op_pokemon, damage, target_index)
                    score += base_score
