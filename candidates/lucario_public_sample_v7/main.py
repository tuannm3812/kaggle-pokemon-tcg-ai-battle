# Public competition-author Mega Lucario ex sample baseline, v7 local Crustle-guard tweak.
# Source reference: https://www.kaggle.com/code/kiyotah/a-sample-rule-based-agent-mega-lucario-ex-deck
# Local adaptation: packaging cells removed; deck.csv is staged beside this file.
# v7 tweak: keep v3 safety and add conditional Crustle-wall Hariyama guard.
...
# Load deck.csv beside this module or from the Kaggle simulation runtime.
deck_candidates = []
module_file = globals().get("__file__")
if module_file:
    deck_candidates.append(Path(module_file).resolve().with_name("deck.csv"))
deck_candidates.extend([Path("deck.csv"), Path("/kaggle_simulations/agent/deck.csv")])
deck_path = next((candidate for candidate in deck_candidates if candidate.exists()), None)
if deck_path is None:
    raise FileNotFoundError("Could not locate deck.csv beside the agent or at runtime paths.")
my_deck = [int(line.strip()) for line in deck_path.read_text().splitlines() if line.strip()]
if len(my_deck) != 60:
    raise ValueError(f"Expected 60 cards in {deck_path}, found {len(my_deck)}.")
...
Dwebble = 344  # public Crustle wall line
Crustle = 345  # public Crustle wall line
Dusk_Ball = 1102  # ×4
Switch = 1123  # ×2
...
def agent(obs_dict: dict) -> list[int]:
    ...
    for card in state.stadium:
        stadium_id = card.id
    low_deck = my_state.deckCount <= 8
    opponent_has_crustle_wall = any(
        pokemon is not None and pokemon.id in {Dwebble, Crustle}
            ...
            for i, my_pokemon in enumerate(my_cards):
                ...
                for a in range(2):
                    ...
                    for j, op_pokemon in enumerate(op_cards):
                        ...
                        if opponent_has_crustle_wall and my_pokemon.id == Mega_Lucario_ex and op_pokemon.id == Crustle:
                            continue
    ...
    def energy_score(pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id == Makuhita or pokemon.id == Hariyama:
            ...
            if opponent_has_crustle_wall:
                ...
                score += 100
            if attacker2 and not opponent_has_crustle_wall:
                score -= 50
        ...
        if o.type == OptionType.NUMBER:
            ...
            if card != None:
                ...
                if context == SelectContext.SWITCH or context == SelectContext.TO_ACTIVE:
                    ...
                    if card.id == Makuhita:
                        if opponent_has_crustle_wall:
                            score += 80 if field_counts[card.id] < 2 else -20
                        ...
                        if opponent_has_crustle_wall:
                            score += 120 if field_counts[Makuhita] >= 1 else -5
                        elif field_counts[Makuhita] >= 1:
                            score += 20
                        else:
                            score -= 20
            ...
            if card.id == 1267:  # Lumiose City
                score = 1
            elif card.id == Lunatone and low_deck:
                score = -1
            else:
                score = 30000
        elif o.type == OptionType.RETREAT:
            ...
            if (
                opponent_has_crustle_wall
                and my_state.active
                and op_state.active
                and my_state.active[0].id == Mega_Lucario_ex
                and op_state.active[0].id == Crustle
                and plan.target < 0
            ):
                score = -1
            else:
