...
def _attach_from_score(obs: object, option: object) -> int:
...
def _choose_indices(obs: object) -> list[int]:
    ...
    elif context == SelectContext.ATTACH_FROM and all(option.type == OptionType.CARD for option in options):
        indexed.sort(
            key=lambda pair: (
                -_attach_from_score(obs, pair[1]),
                _stable_key(pair[1], pair[0]),
            )
        )
    else:
