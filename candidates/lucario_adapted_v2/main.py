"""Repo-owned Mega Lucario archetype candidate v2.

This candidate uses the reconstructed public Lucario decklist but implements a
clean deterministic policy in our own style. V2 keeps the v1 scaffold and adds
explicit target planning, Boss/Switch sequencing, and stronger setup/search
priorities.
"""
        ...
        for attacker_index, attacker in enumerate(cards):
            ...
            for attack_id, required, damage in _attack_options(attacker_id, has_lunatone):
                ...
                for target_area, target_index, target in opponent_targets:
                    ...
                    score += 220 if attacker_area == AreaType.ACTIVE else -180
                    score += 700 if attacker_id == MEGA_LUCARIO_EX else 220 if attacker_id == HARIYAMA else 0
                    if needs_switch:
                        score -= 350
                    if needs_boss:
                        score -= 250
...
def _main_score(obs: object, option: object, plan: AttackPlan) -> int:
    ...
    if option.type == OptionType.EVOLVE:
        ...
        if plan.confident and plan.needs_switch and plan.ready:
            base += 5_500
        else:
            base -= 5_000
        ...
        if card is not None:
            card_id = int(card.id)
            if card_id == SWITCH:
                base += 5_500 if plan.needs_switch and plan.ready else -3_000
...
def _card_score(obs: object, option: object, plan: AttackPlan) -> int:
    ...
    if context == SelectContext.SETUP_ACTIVE_POKEMON:
        ...
        if card_id == SOLROCK and not going_first:
            return 780
        if card_id == RIOLU:
            return 720
        if card_id == SOLROCK:
    ...
    if context == SelectContext.TO_HAND:
        ...
        if card_id == FIGHTING_ENERGY:
            return 750 if plan.energy_needed > 0 else 420
        if card_id == BOSS_ORDERS and plan.needs_boss:
            return 900
        if card_id == SWITCH and plan.needs_switch:
            return 900
